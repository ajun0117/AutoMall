...
-(void)reloadDataLevel:(NSInteger)level {
...
//    NSArray *keyArray = [[newDic allKeys] sortedArrayUsingSelector:@selector(compare:)]; //排序后的keys
//    return keyArray;
    NSArray *sectionIndexTitles = nil;
    if ([self.dataSource respondsToSelector:@selector(multiTablesView:sectionIndexTitlesAtLevel:)]) {
		sectionIndexTitles = [self.dataSource multiTablesView:self sectionIndexTitlesAtLevel:[self indexOfTableView:tableView]];
	}
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.dataSource respondsToSelector:@selector(multiTablesView:level:canEditRowAtIndexPath:)]) {
        isEdit = [self.dataSource multiTablesView:self level:[self indexOfTableView:tableView] canEditRowAtIndexPath:indexPath];
    }
    return isEdit;
}
