...
- (void)reloadData{
    ...
    if (num) {
        ...
        if (num != 1) {
            self.pageControl.frame = CGRectMake(self.width - 15*num - 10, self.height - 20, 15*num, 20);
            self.pageControl.numberOfPages = num;
            [self.scrollView setContentSize:CGSizeMake(self.width*3, self.height)];
            if (! timer) {
                timer = [NSTimer scheduledTimerWithTimeInterval:DefineAdTime target:self selector:@selector(cycleClick:) userInfo:nil repeats:YES];
            } 
        }
...
//    self.bgImageView.image = [UIImage imageNamed:DefineAdImage];
    ...
    _titleL.text  = [self.delegate titleStringInAdView:self index:0];
...
- (void)initScrollViewSubViewsWithSelectNum:(NSInteger)num{
    ...
    if (self.isHomeAd) {
        defaultImageName = HomeDefineAdImage;
    }
    else {
        defaultImageName = ListDefineAdImage;
    }
    [v2 sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[self.delegate imageUrlInAdView:self index:_sIndex]]] placeholderImage:[UIImage imageNamed:defaultImageName]];
    [v1 sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[self.delegate imageUrlInAdView:self index:lasttag]]] placeholderImage:[UIImage imageNamed:defaultImageName]];
    [v3 sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[self.delegate imageUrlInAdView:self index:nexttag]]] placeholderImage:[UIImage imageNamed:defaultImageName]];
