...
- (void)setUpFrames
{
    ...
    _titleL.bounds = CGRectMake(0, 0, kAPPWidth, 30);
    _titleL.center = CGPointMake(kAPPWidth * 0.5, kAppHeight - 80);
    _saveButton.frame = CGRectMake(kAPPWidth - 70, 15, 55, 30);
}
...
//    titleL.bounds = CGRectMake(0, 0, 100, 40);
//    titleL.center = CGPointMake(kAPPWidth * 0.5, 30);
//    titleL.layer.cornerRadius = 15;
//    titleL.clipsToBounds = YES;
    
    titleL.text = self.currentImageTitle;
    _titleL = titleL;
...
//        UIView *view = (UIView *)parentView;
        targetTemp.origin.y =  targetTemp.origin.y + 64;
    }
...
#pragma mark 获取当前图片标题
- (NSString *)titleStringForIndex:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:titleStringForIndex:)]) {
        return [self.delegate photoBrowser:self titleStringForIndex:index];
    }
    return nil;
}
